...
- **MCP configuration** — CRUD over each user's registered MCPs (name, local port, optional sub-path).
...
- The agent is the only part of the app that ever talks to `localhost`; it holds the WebSocket tunnel and the local name → port map.
...
- Register a local MCP with a name, its local port, and an optional sub-path.
